...
Story_nodes.cpp is where all the magic happens, it manually maps together the story. It creates nodes and declared what nodes are connected to what. Reading trough this file might be a bit confusing but essentially what it does is the following:
...
```
Current = Begging_node
while we are not finished:
  current = run code in current node (run() function)
```

At each run as you can see in the extra features you can store the results. These results are saved in the `storage.txt` file. Every line in this file stores various information such as Name of the player, what level he was at, what node he was located when he quit...

`game_loop.cpp` has the main() function of this program, the logic there is printing the base menu allowing the user to either start a new game, or load an old game. If a old game is loaded it reads all entries from the storage file and allows the user to choose. After all of this the game loop showed above takes place
